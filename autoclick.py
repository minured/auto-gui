...
def get_xy(img_model_path):
    ...
    # 画框
    cv2.rectangle(img, (upper_left[0], upper_left[1]),
                  (lower_right[0], lower_right[1]), (0, 255, 0), 2)

    # return upper_left

    # 计算坐标的平均值并将其返回
    avg = (int(((upper_left[0] + lower_right[0]) / 2) * x_scale),
           int(((upper_left[1] + lower_right[1]) / 2) * y_scale))
    ...
    logger.log('已找到坐标{}，{}'.format(avg, img_model_path))
    return avg


def switchToWindow(app_name):
    logger.log('正在匹配窗口 {}'.format(app_name))
    for proc in psutil.process_iter(['pid', 'name']):
        try:
            if app_name.lower() in proc.info['name'].lower():
                ...
                time.sleep(1)
                logger.log('已切换到{}'.format(proc.info['name']))
                return True
        except (psutil.NoSuchProcess, psutil.AccessDenied, psutil.ZombieProcess):
            logger.log('没匹配到窗口 {}'.format(app_name))
            pass
...
def auto_Click(var_avg):
    ...
    :return: None
    """
    logger.log('正在移动到 {}'.format(var_avg))
    pyautogui.moveTo(var_avg[0], var_avg[1], duration=2)
    logger.log('左击 {}'.format(var_avg))
